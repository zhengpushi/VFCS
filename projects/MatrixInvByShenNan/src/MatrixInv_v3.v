...
Module MatrixInv (B : BaseType) (E : EqDecidableFieldElementType B).
  ...
  Lemma diagonalOnes_ext : forall {n} (MA : smat n) (x y: nat),
      diagonalOnes MA x -> y <= x -> diagonalOnes MA y.
  Proof.
    intros. hnf in *; intros. apply H; auto; try lia.
  Qed.
  
  (* 归一化上三角形：对角线全1，左下角全0 *)
  Definition normedLowerTri {n} (MA: smat n) := 
  ...
  Lemma mrowAdd_keep_normedLowerTri:
    forall {n} (MA : smat n) (x i' : nat),
      x < n -> i' < x -> normedLowerTri MA -> 
      normedLowerTri ((mrowAdd i' x (- (MA $ i' $ x)))%A * MA).
  Proof.
    intros. unfold normedLowerTri. inv H1. split; hnf in *; intros.
  ...
  Fixpoint firstNonzero {n} (MA: smat n) (i: nat) (y: nat) : option nat :=
    match i with
    | O => None
    | S i' =>
  ...
  Lemma firstNonzero_max: forall {n} (MA : smat n) (i j : nat) (r : nat),
      firstNonzero MA i j = Some r -> r <= n.
  Proof.
    intros n MA i. revert MA. induction i; intros; simpl in *. easy.
    destruct (decidable (MA $ (n - S i) $ j) 0).
    - apply IHi in H; auto.
    - inversion H. lia.
  Qed.

  Lemma firstNonzero_max_ext: forall {n} (MA : smat n) (i j : nat) (r : nat),
      i < n -> firstNonzero MA i j = Some r -> r < n.
  Proof.
    intros n MA i. revert MA. induction i; intros; simpl in *. easy.
    destruct (decidable (MA $ (n - S i) $ j) 0).
    - apply IHi in H0; auto. lia.
    - inversion H0. lia.
  Qed.
  
  (* 非零元行号最小值是 n - i *)
  Lemma firstNonzero_min: forall {n} (MA : smat n) (i j : nat) (r : nat),
      firstNonzero MA i j = Some r -> r >= n - i.
  Proof.
    intros n MA i. revert MA. induction i; intros; simpl in *; try easy.
    destruct (decidable (MA $ (n - S i) $ j) 0).
  ...
  (* 若MA[y,y]=1，则从(cur,y)向下消元后，前n-cur行的所有元素保持不变 *)
  Lemma elimDown_former_row_keep : forall {n} (MA : smat n) (y x : nat) i j,
      y < n -> x < n - y -> MA $ y $ y = 1 -> i < n - x -> j < n ->
      (snd (elimDown MA x y)) $ i $ j = MA $ i $ j.
  Proof.
    intros n MA y x. revert MA y.
    induction x; auto. intros. simpl.
  ...
  Lemma elimDown_latter_row_0 : forall {n} (MA : smat n) (x y : nat) i,
      y < n -> x < n - y -> MA $ y $ y = 1 ->
      i < n -> i >= n - x ->
      (snd (elimDown MA x y)) $ i $ y = 0.
  Proof.
    intros n MA x y. revert MA y.
    induction x; intros; simpl; try lia.
    bdestruct (Nat.eqb i (n - S x)).
    - destruct elimDown eqn:eq. simpl. rewrite (pair_eq_imply_snd eq).
      ...
      + rewrite mnth_mrowAdd_mul; auto; try lia. elim_bool.
    - destruct elimDown eqn: eq. simpl. rewrite (pair_eq_imply_snd eq).
      apply IHx; auto; try lia. rewrite mnth_mrowAdd_mul; auto; try lia.
      elim_bool.
  Qed.

  (* 向下消元后的前x列左下方都是0 *)
  Lemma elimDown_keep_lowerLeftZeros: forall {n} (MA : smat n) (y x : nat),
      y < n -> x < n - y -> MA $ y $ y = 1 -> lowerLeftZeros MA y -> 
      lowerLeftZeros (snd (elimDown MA x y)) y.
  Proof.
    intros n MA y x. revert MA y. induction x; intros; simpl; auto.
    destruct elimDown eqn: eq. simpl. rewrite (pair_eq_imply_snd eq).
  ...
  Fixpoint rowEchelon {n} (MA: smat n) (i: nat) : option (smat n * smat n) :=
    ...
    | S i' =>
        ...
        match firstNonzero MA i (n - i) with
        | None => None
  ...
  Proof.
    ...
    - unfold rowEchelon in H0; fold (@rowEchelon n) in H0.
      (* 非零元行号 =? n *)
      destruct firstNonzero as [r|]; try easy.
      (* elimDown的结果 *)
      ...
      rewrite <- H01, <- H02, <- H2. rewrite !mmul_assoc.
      (* 使用 elimDown不变式 *)
  ...
  Proof.
    ...
    - unfold diagonalOnes. intros.
      ...
      destruct firstNonzero as [r|] eqn:Hr; try easy.
      destruct (elimDown) eqn: Heqp.
      destruct (rowEchelon s0 i) as [[E3 MT']|] eqn: Heqo; try easy.
      remember (mrowSwap (n - S i) r * X) as A0.
      remember (mrowScale (n - S i) (/ (A0 $ (n - S i) $ (n - S i))) * A0) as A1.
      inversion H1. clear H1. apply IHi in Heqo; clear IHi; auto; try lia.
      + rewrite <- H6. auto.
      + unfold diagonalOnes. intros.
        ...
        * rewrite HeqA1.
          ...
          ** rewrite H7. rewrite inv_self; auto.
          ** rewrite HeqA0.
             pose proof (firstNonzero_min  _ _ _ Hr).
             pose proof (firstNonzero_max  _ _ _ Hr).
             rewrite mnth_mrowSwap_mul; auto; try lia.
             *** elim_bool.
                 apply diagonalOnes_ext with (y:=S i1) in H0; try lia.
                 apply H0; auto.
             *** apply firstNonzero_max_ext in Hr; auto. lia.
        * rewrite HeqA1.
          rewrite mnth_mrowScale_mul; try lia. elim_bool.
  ...
  Proof.
    ...
    - unfold rowEchelon in H1; fold (@rowEchelon n) in H1.
      destruct (firstNonzero) as [r|] eqn: Hr; try easy.
      ...
      + unfold lowerLeftZeros in *; intros.
        ...
        * rewrite !mnth_mrowSwap_mul; auto; try lia.
          ** elim_bool. rewrite (H0 _ j0); try lia. ring.
             apply firstNonzero_max_ext in Hr; auto; try lia.
             apply firstNonzero_min in Hr. lia.
          ** apply firstNonzero_max_ext in Hr; auto; try lia.
          ** apply firstNonzero_max_ext in Hr; auto; try lia.
        * rewrite !mnth_mrowSwap_mul; auto; try lia.
  ...
  Theorem rowEchelon_correct: forall {n} (A E EA : smat n),
      rowEchelon A n = Some (E, EA) -> E * A == EA /\ normedLowerTri EA.
  Proof.
    intros. split.
    - apply rowEchelon_mult_eq in H; eauto.
    - unfold normedLowerTri. split.
  ...
  (* ************************************************************************ *)
  (* 向上消元后仍然是下三角矩阵 *)
  Lemma elimUp_keep_normedLowerTri : forall {n} (MA : smat n) (i y : nat),
      y < n -> i <= y -> normedLowerTri MA ->
      normedLowerTri (snd (elimUp MA y i)). 
  Proof.
    intros n MA i y. revert MA y. induction i; intros; auto. simpl.
    destruct elimUp as [E MT] eqn: H2.
    simpl. rewrite (pair_eq_imply_snd H2).
    apply IHi; auto; try lia.
    apply mrowAdd_keep_normedLowerTri; auto.
  Qed.

  (* 上消元的元素保持性 *)
  Lemma elimUp_lower_rows_keep : forall {n} (MA : smat n) (y i : nat) i' j,
      y < n -> i <= y -> i' < n -> i' >= i -> j < n ->
      (snd (elimUp MA y i)) $ i' $ j = MA $ i' $ j.
  Proof.
    intros n MA y i. revert MA y. induction i; intros; auto. simpl.
  ...
  (* 上消元后该列上方元素为 0 *)
  Lemma elimUp_upper_rows_to_0 : forall {n} (MA : smat n) (y i : nat) i',
      y < n -> i <= y -> normedLowerTri MA -> i' < i ->
      (snd (elimUp MA y i)) $ i' $ y = 0.
  Proof.
    intros n MA y i. revert MA y. induction i; intros; auto; try lia. simpl.
    ...
    - apply IHi; auto; try lia. apply mrowAdd_keep_normedLowerTri; auto; try lia.
  Qed.

  (* 上消元后，右上角全是 0 *)
  Lemma elimUp_keep_upperRightZeros: forall {n} (MA : smat n) (y i L : nat),
      y < n -> i <= y -> L < n - y -> normedLowerTri MA -> upperRightZeros MA L ->  
      upperRightZeros (snd (elimUp MA y i)) L.
  Proof.
    intros n MA y i. revert MA y. induction i; intros; auto. simpl.
    destruct elimUp as [E MT] eqn: H4. simpl. rewrite (pair_eq_imply_snd H4).
    apply IHi; auto; try lia.
    - apply mrowAdd_keep_normedLowerTri; auto; lia.
    - hnf; intros. rewrite mnth_mrowAdd_mul; auto; try lia. elim_bool.
      subst. rewrite !(H3 _ j); auto; try lia. ring.
  Qed.

  (* 上消元后，右上角元素都是 0 *)
  Lemma elimUp_keep_upperRightZeros_entend: forall {n} (MA : smat n) (y : nat),
      y < n -> normedLowerTri MA -> upperRightZeros MA (n - y - 1) -> 
      upperRightZeros (snd (elimUp MA y y)) (n - y).
  Proof.
    intros. hnf; intros.
    bdestruct (j =? y).
    - subst. apply elimUp_upper_rows_to_0; auto; lia.
    - rewrite elimUp_keep_upperRightZeros
        with (L := (n - y - 1)%nat); auto; lia.
  Qed.
  ...
  Lemma minRowEchelon_keep_upper_triangle: forall {n} (MA : smat n) (i : nat),
      i <= n -> normedLowerTri MA -> normedLowerTri (snd (minRowEchelon MA i)).
  Proof.
  ...
  Lemma minRowEchelon_eq_I : forall {n} (MA : smat n),
      normedLowerTri MA -> snd (minRowEchelon MA n) == I.
  Proof.
