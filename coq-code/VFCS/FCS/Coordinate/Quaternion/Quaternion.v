...
Section quat_def.
  ...
  (* 四元数 q = w + x i + y j + z k, 可看作以 {1, i, j, k} 为基(Basis)的
  ...
  Proof.
    intros. split; intros.
    - destruct H as [? H]; destruct H as [? H]; destruct H as [? H].
      subst; auto.
...
(*       (* 自动证明 *) lra. *)
  ...
  Definition quat_of_s_v (w : R) (v : V3) :=
  ...
  Definition quat_of_v3 (v : V3) : quat := quat_of_s_v 0 v.
  ...
  Definition quat_of_v4 (v : V4) : quat :=
    let '(w,x,y,z) := v4_to_t4 v in
      mk_quat w x y z.
  ...
  (** Quaternion to V4 *)
  Definition v4_of_quat (q : quat) : V4 :=
    let '(w,x,y,z) := (w q, x q, y q, z q) in
      Matrix.mk_mat_4_1 w x y z.
  
End quat_def.


(* ############################################################### *)
(** * Customize Tactical for proof *)
...
(*   let w : R := c * (Re q) in
  let v : R * R * R := Im q in
  let v3 : array 3 := a_cmul c (t3_to_v3 v) in
    quat_from_s_v w v3. *)
...
Notation "a c* q" := (qcmul a q) 
  (at level 45, left associativity) : quat_scope.
...
Definition qmulVEC (p q : quat) : quat :=
  let p0 : R := Re p in
  let q0 : R := Re q in
  let pv : V3 := t3_to_v3 (Im p) in
  let qv : V3 := t3_to_v3 (Im q) in
  let w : R := (p0 * q0 - scalar_of_mat (qv ⊤ × pv))%R in
  let v : V3 := (v3cross pv qv + p0 c* qv + q0 c* pv)%M in
    quat_of_s_v w v.
...
Proof.
  exists (quat_of_tuple4 (0,1,2,1)).
  exists (quat_of_tuple4 (0,2,1,2)).
...
Definition qconj (q : quat) : quat :=
  let w : R := Re q in
  let v : V3 := mopp (t3_to_v3 (Im q)) in
    quat_of_s_v w v.
...
(* Im (q * q∗) = (0,0,0) *)
Lemma qmul_qconj_Re_eq_0 (q : quat) : Im (q * q∗) = (0,0,0).
Proof.
  destruct q. lqa.
Qed.
...
(* 0 <= qnorm2 q *)
Lemma zero_le_qnorm2 : forall (q : quat), 0 <= qnorm2 q.
Proof.
...
(* 0 <= ‖ q ‖ *)
Lemma zero_le_qnorm : forall (q : quat), 0 <= qnorm q.
Proof.
...
(* 范数不等于0 iff 范数平方不等于0 *)
Lemma qnorm_neq0_iff_qnorm2_neq0 : forall q, ‖ q ‖ <> 0 <-> qnorm2 q <> 0.
Proof.
...
Lemma qmul_qinv_unitary : forall (q : quat), ‖ q ‖ <> 0 ->
  q * (qinv q) = quat_of_s 1.
Proof.
  intros. destruct q. rewrite qnorm_neq0_iff_qnorm2_neq0 in H. lqa; auto.
Qed.

Lemma qmul_qinv_unitary_rev : forall (q : quat), ‖ q ‖ <> 0 ->
...
Definition qunit2 (q : quat) : Prop := qnorm2 q = 1.
...
Lemma vec_rot_by_quat_keep_norm : forall (pv : V3) (q : quat) (H : qunit q),
...
Lemma vec_rot_by_quat_keep_norm_ext : forall (pv : V3) (s : R) (q : quat) 
  (H : qunit q) (H1 : s <> 0),
  let q' := s c* q in
  let pv' := vec_rot_by_quat q' pv in
  v3norm pv = v3norm pv'.
...
(* 根据两个单位来计算夹角、转轴。*)
Definition rot_angle_by_twovec (v0 v1 : V3) : R := 
  acos (scalar_of_mat (v0 ⊤ × v1)).

Definition rot_axis_by_twovec (v0 v1 : V3) : V3 :=
...
四元数 *)
Definition qrot_by_two_vec_ops (v0 v1 : V3) : quat :=
  quat_of_s_v (scalar_of_mat (v0 ⊤ × v1)) (v3cross v0 v1).


(* (* 若单位向量v0和v1的夹角是 θ/2，且不共线，则由它们生成的垂直方向的向量v有确定形式 *)
Lemma gen_vec_by_v0_v1_eq : forall (v0 v1 : V3) (θ : R) (H1 : v3norm v0 = 1)
