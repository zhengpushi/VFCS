...
Module Module_quat (E : EType).
  ...
  Definition quat_from_tuple4 (t : t4) : quat :=
    let '(w,x,y,z) := t in
      mk_quat w x y z.
  Definition quat_to_tuple4 (q : quat) : t4 :=
    (w q, x q, y q, z q).

  (* Construct a quaternion by a scalar number and a 3-dim vector *)
  Definition quat_from_s_v (w : T) (v : v3) :=
  ...
  Definition quat_from_v (v : v3) : quat := quat_from_s_v 0 v.
  ...
  *)
  ...
  Definition quat_mulc (q : quat) (c : T) : quat :=
    let w : T := (Re q) * c in
    let v : T * T * T := Im q in
    let v3 : array 3 := a_mulc (t3_to_v3 v) c in
      quat_from_s_v w v3.
  ...
  Definition quat_mul_formu1 (q1 q2 : quat) : quat :=
    let w1 : T := Re q1 in
    let w2 : T := Re q2 in
    let v1 : array 3 := t3_to_v3 (Im q1) in
    let v2 : array 3 := t3_to_v3 (Im q2) in
    let v1m : matrix 3 1 := cv_to_m v1 in
    let v2m : matrix 3 1 := cv_to_m v2 in
    let w : T := (w1 * w2 - m_1x1_to_s (m_mul (v2m ') v1m))%element in
    let xyz : T * T * T := m_3x1_to_t3 (m_add (cv3_cross v1 v2)
  ...
  Definition quat_PLUS (q : quat) : matrix 4 4 :=
    let w : T := Re q in
    let v : array 3 := t3_to_v3 (Im q) in
  ...
  Definition quat_mul_formu_PLUS (q1 q2 : quat) : quat :=
    ...
    let m1 : matrix 4 1 := cv_to_m (t4_to_v4 (quat_to_tuple4 q2)) in
    let m2 : matrix 4 1 := m_mul PLUS m1 in
    let '(w,x,y,z) := v4_to_t4 (m_rx1_to_a m2) in
      mk_quat w x y z.
  ...
  Definition quat_mul_formu_MINUS (q1 q2 : quat) :=
    ...
    let m1 : matrix 4 1 := cv_to_m (t4_to_v4 (quat_to_tuple4 q1)) in
  ...
  *)
  ...
  Lemma quat_mul_two_v3 (u v : array 3) :
    ...
    let q : quat :=  quat_from_s_v
      (m_1x1_to_s (m_neg (m_mul (m_trans (cv_to_m u)) (cv_to_m v)))) 
  ...
  Definition quat_conjugate (q : quat) : quat :=
    let w : T := Re q in
    let v : array 3 := a_neg (t3_to_v3 (Im q)) in
      quat_from_s_v w v.
  ...
  *)
  ...
  Definition quat_norm_formu3 (q : quat) : T :=
    let q0 : T := Re q in
    let qv : array 3 := t3_to_v3 (Im q) in
      (q0 * q0) + (m_1x1_to_s (m_mul (cv_to_m qv ') (cv_to_m qv))).
