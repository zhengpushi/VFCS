...
(** ** Variables *)  
Parameter rho : R.              (* 飞行环境空气密度，单位：kg/m³ *)
Parameter P_a : R.              (* 大气压强，单位：Pa *)

(** ** Predication for parameters checking *)
Definition is_rho r := rho = r.
Definition is_h r := h = r.
Definition is_rho0 r := rho0 = r.
Definition is_P_a r := P_a = r.
...
(** 空气密度模型, 4.3 *)
Axiom rho_def : rho = (T0 * P_a) / (P_a0 * (T0 + T_t)) * rho0.
...
Parameter C_d : R. (* 叶片截面翼型的阻力系数，单位：无 *)
Parameter C_T : R. (* 推力系数, 单位：无 *)
Parameter C_M : R. (* 扭矩系数，单位：无 *)
...
(** 4.1 螺旋桨拉力计算模型 *)
Axiom T_def1 : T = C_T * rho * (N / 60)² * (D_p ^ 4).

(** 4.2 螺旋桨扭矩计算模型 *)
Axiom M_def1 : M = C_M * rho * (N / 60)² * (D_p ^ 5).
...
(** 4.51 叶片截面翼型的阻力系数定义 *)
Axiom C_d_def : C_d = C_fd + (1 / (PI * A * e)) * (C1²).

(** 4.52 叶片升力定义 *)
Axiom L_def : L = (1/2) * C1 * rho * S_sa * W².
...
(** 4.55 螺旋桨扭矩的另一个模型 *)
Axiom M_def2 : M = (1/4) * rho * B_p * C_d * W² * S_sa * D_p.

(** p79 下洗气流引起的阻生角定义 *)
Axiom gamma_def : gamma = atan (C_d / C1).
...
Parameters
  ...
  G_m     (* 电机重量，单位：kg *)
  (* 变量 *)
  N_m0    (* 空载转速，单位：rpm = r/min *)
  K_E     (* 电机反电动势常数，单位：v/rpm *)
  K_T     (* 电机转矩常数，单位：N.m *)
  U_m     (* 电机等效输入电压，单位：V *)
...
Definition is_K_E r := K_E = r.
Definition is_K_T r := K_T = r.
...
(** 电机反电动势 *)
Axiom E_a_def : E_a = K_E * N.

(** 电机电磁转矩 *)
Axiom T_e_def : T_e = K_T * I_m.

(** 电机输出转矩，等于电磁转矩与空载转矩之差 *)
Axiom M_def3 : M = T_e - T_m0.

(** 电机输出转矩，等于螺旋桨转矩 *)
Axiom M_def4 : M = K_T * I_a.

(** 电机空载转矩与实际空载电流成正比 *)
Axiom T_m0_def : T_m0 = K_T * I_0_hat.

(** 标称空载情况下的关系式 *)
Axiom U_m0_def : U_m0 = K_E * N_m0 + I_m0 * R_m.

(** p81，对于多旋翼电机，电机实际空载电流与标称空载电流差别不大 *)
Axiom I_0_hat_def : I_0_hat = I_m0.

(** 4.63，电机空载转速 *)
Axiom N_m0_def : N_m0 = K_V0 * U_m0.

(** 4.65，电机转矩常数与电机反电动势常数的关系 *)
Axiom K_T_and_K_E : K_T = val_9_55 * K_E.
