...
(** *** matrix addition *)
Definition madd {r c} (m1 m2 : mat r c) : mat r c := madd m1 m2 (Aadd:=Qcplus).
...
(** *** matrix opposition *)
Definition mopp {r c} (m : mat r c) : mat r c := mopp m (Aopp:=Qcopp).
...
Definition msub {r c} (m1 m2 : mat r c) : mat r c :=
  msub m1 m2 (Aadd:=Qcplus)(Aopp:=Qcopp).
...
Definition mcmul {r c} a (m : mat r c) : mat r c := mcmul a m (Amul:=Qcmult).
...
Definition mmul {r c s} (m1 : mat r c) (m2 : mat c s) : mat r s :=
  mmul m1 m2 (Aadd:=Qcplus) (A0:=0) (Amul:=Qcmult).
