 ...
 *)
Class SubVectorSpace `{VS : VectorSpace} {H} {h2v : H -> V} {v2h : V -> H}
  `{Injective H V h2v} :=
  {
...
Section vlcomb.
  ...
  Definition vlcomb {n} (coefs : @vec A n) (vecs : @vec V n) : V :=
    vsum (vmap2 Vcmul coefs vecs).

  (* 0 * v1 + 0 * v2 + ... + 0 * vn = 0 *)
  Lemma vlcomb_c0_eq0 : forall {n} (vecs : @vec V n), vlcomb vzero vecs = 0.
  Proof.
  ...
  (** (c1 + c2) * v = c1 * v + c2 * v *)
  Lemma vlcomb_coef_add : forall {n} (vecs : @vec V n) c1 c2,
      vlcomb (c1 + c2)%V vecs = vlcomb c1 vecs + vlcomb c2 vecs.
  Proof.
  ...
  (** (- c) * v = - (c * v) *)
  Lemma vlcomb_coef_opp : forall {n} (vecs : @vec V n) c,
      vlcomb (- c)%V vecs = - (vlcomb c vecs).
  Proof.
  ...
  (** (c1 - c2) * v = c1 * v - c2 * v *)
  Lemma vlcomb_coef_sub : forall {n} (vecs : @vec V n) c1 c2,
      vlcomb (c1 - c2)%V vecs = vlcomb c1 vecs - vlcomb c2 vecs.
  Proof.
...
Section vldep.
  ...
  Definition vldep {n} (vecs : @vec V n) : Prop :=
    exists (coefs : @vec A n), coefs <> vzero /\ vlcomb coefs vecs = 0.

  (* Vectors v1, v2, ..., vn are linearly independent *)
  Definition vlindep {n} (vecs : @vec V n) : Prop := ~ (vldep vecs).

  (* 向量线性无关，iff，线性组合的方程组只有唯一的零解。即：只要有解，必都为0。*)
  Lemma vlindep_iff_unique0 : forall {n} (vecs : @vec V n),
      vlindep vecs <-> forall coefs, vlcomb coefs vecs = 0 -> coefs = vzero.
  Proof.
    intros. unfold vlindep, vldep. split; intros.
    - apply not_ex_all_not with (n:=coefs) in H. apply not_and_or in H.
  ...
  Lemma vlindep_imply_coef_uniq : forall {n} (vecs : @vec V n),
      vlindep vecs -> (forall c1 c2, vlcomb c1 vecs = vlcomb c2 vecs -> c1 = c2).
  Proof.
...
Section vspan.
  ...
  Definition vspan {n} (vecs : @vec V n) : Prop :=
    forall (u : V), (exists (coefs : @vec A n), vlcomb coefs vecs = u).
End vspan.
...
Section vbasis.
  ...
  Definition vbasis {n} (vecs : @vec V n) : Prop :=
    vlindep vecs /\ vspan vecs.
  ...
  (** Elements v1, v2, . . . , vn of a vector space V constitute a basis of 
      ...
      u = c1v1 + c2v2 + · · · + cnvn  *)
  Theorem vbasis_if_unique_coefs : forall {n} (vecs : @vec V n),
      vbasis vecs <-> forall u : V, exists! (coefs : @vec A n), vlcomb coefs vecs = u.
  Proof.
    intros. split; intros.
...
Section vltrans.
  ...
  (** T (c1v1 + c2v2 + · · · + cnvn) 
      ...
      = c1w1 + c2w2 + · · · + cnwn *)
  Lemma vltrans_linear : forall {n} (T : V -> W) (coefs : @vec A n)
                           (v : @vec V n) (w : @vec W n),
      vltrans T -> (forall i, w$i = T(v$i)) ->
      T (vlcomb (Vadd:=Vadd)(Vzero:=Vzero)(Vcmul:=Vcmul) coefs v) =
        vlcomb (Vadd:=Wadd)(Vzero:=Wzero)(Vcmul:=Wcmul) coefs w.
  Proof.
    intros. unfold vlcomb.
    apply eq_trans with
      (vsum (Aadd:=Wadd)(Azero:=Wzero) (vmap T (vmap2 Vcmul coefs v))).
