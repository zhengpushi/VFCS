...
Section sec_1_1.
  ...
  Definition lc {n : nat} (v w : vec n) (c d : R) := c c* v + d c* w.

  (* Example  *)
  Let v : vec 2 := mk_vec2 1 1.
  Let w : vec 2 := mk_vec2 2 3.
  ...
  Section special_linear_combinations.
    ...
    (* cv + 0w = c.v *)
    Lemma lc_eq_scal : forall {n} (v w : vec n) c, lc v w c 0 == c c* v.
    Proof. lma. Qed.
  ...
  Definition lc {n : nat} (l : list (R * (vec n))%type) :=
    let l' := map (fun x : R * vec n => let (c,v) := x in c c* v) l in
  ...
  Section angle.
    ...
    Section perp.
      ...
      Axiom PythagorasLaw2D : forall (a b : vec 2),
          let c := a - b in
          a ⟂ b <-> (Rsqr (v|a|) + Rsqr (v|b|))%R = Rsqr (v|c|).
    ...
    Section angle.
      ...
      Proof.
        ...
        - (* v = 0, w = 0 *)
          assert (v ⋅ w = 0).
          (*   { rewrite m. rewrite vdot_0_l. easy. } *)
